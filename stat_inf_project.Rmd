...
I will first perform an ANOVA on the regions.  The hypotheses are:
...
The high F value and very low p value indicate that we should reject the null hypothesis that there is no difference average contstant income between the regions.  The data suggest that there is a significant difference in the constant income between at least two of the reagions.


## Region by Region t-tests

I want to discover which regions have significant differences in constant income, so I will run a t-test on all possible combinations of regions, using the bonferoni correction to control for type 1 errors.
...
A close inspection of the table shows that the East South Central and West South Central regions report lower average constant incomes for black households than the other seven regions.  The states included in these regions are Alabama, Arkansas, Kentucky, Louisiana, Mississippi, Oklahoma, Tennessee and Texas.

A map and further information on the regions can be found here: https://www2.census.gov/geo/pdfs/maps-data/maps/reference/us_regdiv.pdf

All of the r code for this analysis can be found here: https://github.com/lecroc/Inferential-Statistics
