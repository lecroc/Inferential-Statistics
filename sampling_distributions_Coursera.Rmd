...
samp1 %>%
  summarise(s_mean = mean(area), s_med = median(area), 
            s_sd = sd(area), s_iqr = IQR(area),
            s_min = min(area), s_max = max(area),
            s_q1 = quantile(area, 0.25),  # first quartile, 25th percentile
            s_q3 = quantile(area, 0.75))  # third quartile, 75th percentile
```
